...
public class MyRandomData implements Iterable<Object> {
	...
	public MyRandomData(Schema schema, int count) {
		this(schema, count, false);
	}

	public MyRandomData(Schema schema, int count, long seed) {
		this(schema, count, seed, false);
	}

	public MyRandomData(Schema schema, int count, boolean utf8ForString) {
		this(schema, count, System.currentTimeMillis(), utf8ForString);
	}

	public MyRandomData(Schema schema, int count, long seed, boolean utf8ForString) {
		this.root = schema;
		this.seed = seed;
		this.count = count;
	...
	public Iterator<Object> iterator() {
		return new Iterator<Object>() {
			...
			public Object next() {
				n++;
				return generate(null, root, random, 0);
			}
	...
	private Object generate(String fakerValue, Schema schema, Random random, int d) {
		...
		case RECORD:
			...
			for (Schema.Field field : schema.getFields()) {
				Object value = (field.getObjectProp(USE_DEFAULT) == null) ? generate(fakerExpression(field.doc()), field.schema(), random, d + 1)
		...
		case INT:
			if (fakerValue != null) {
				return Integer.valueOf(fakerValue);
			}
			return random.nextInt();
		case LONG:
			if (fakerValue != null) {
				return Long.valueOf(fakerValue);
			}
			return random.nextLong();
		case FLOAT:
			if (fakerValue != null) {
				return Float.valueOf(fakerValue);
			}
			return random.nextFloat();
		case DOUBLE:
			if (fakerValue != null) {
				return Double.valueOf(fakerValue);
			}
			return random.nextDouble();
		case BOOLEAN:
			if (fakerValue != null) {
				return Boolean.valueOf(fakerValue);
			}
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			for (Object datum : new MyRandomData(sch, Integer.parseInt(args[2]))) {
				writer.append(datum);
			}
		}
		finally {
