...
public class Microcks {
	...
	private void case0() {
		...
		List<GenericData.Record> users = DataGenerator.generateRecords(
				DataGenerator.uriToSchema("classpath:/avro/microcks/user0.yaml"), 3);

		users.forEach(u -> System.out.println(DataGenerator.toJson(u)));
...
// While those are valid records compliant with the avro schema they are hardly readable. We can do better by annotating the schema with some Data faker hits.
	...
	private void case1() {
		...
		List<GenericData.Record> users = DataGenerator.generateRecords(
				DataGenerator.uriToSchema("classpath:/avro/microcks/user1.yaml"), 3);
...
// Can we make the email field reuse the fullName field when computing the email?
	...
	private void case2() {
		...
		List<GenericData.Record> users = DataGenerator.generateRecords(
				DataGenerator.uriToSchema("classpath:/avro/microcks/user2.yaml"), 3);
...
//		{"id":"200-00-0000","sendAt":1645205272610,"fullName":"Bo Brakus DVM","email":"bo.brakus.dvm@yahoo.com","age":41}
	...
	private void case3() {
		...
		List<GenericData.Record> users = DataGenerator.generateRecords(
				DataGenerator.uriToSchema("classpath:/avro/microcks/user3.yaml"), 5);
		users.forEach(u -> System.out.println(DataGenerator.toJson(u)));
...
// Let's introduce the Click stream data (click.yaml)  as well.
		...
		users.forEach(u -> System.out.println(DataGenerator.toJson(u)));
		List<GenericData.Record> clicks = DataGenerator.generateRecords(
				DataGenerator.uriToSchema("classpath:/avro/microcks/click.yaml"), 5,
				sharedFieldValuesContext);
		System.out.println("---");
		clicks.forEach(c -> System.out.println(DataGenerator.toJson(c)));
...
//		{"user_id":"514-91-4473","page":35713,"action":"cart","user_agent":"Mozilla/4.0 (compatible; MSIE 6.0; AOL 9.0; Windows NT 5.1; SV1; .NET CLR 1.0.3705; .NET CLR 1.1.4322; Media Center PC 4.0)"}
	...
	private void case5() {
		...
		List<GenericData.Record> users = DataGenerator.generateRecords(
				DataGenerator.uriToSchema("classpath:/avro/microcks/user4.yaml"), 5,
				sharedFieldValuesContext);
		users.forEach(u -> System.out.println(DataGenerator.toJson(u)));
		List<GenericData.Record> clicks = DataGenerator.generateRecords(
				DataGenerator.uriToSchema("classpath:/avro/microcks/click1.yaml"), 5,
				sharedFieldValuesContext);
		System.out.println("---");
		clicks.forEach(c -> System.out.println(DataGenerator.toJson(c)));
